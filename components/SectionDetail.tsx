  ...
  useEffect(() => {
    const fetchInfo = async () => {
      ...
      try {
        ...
        setContent("No se pudo cargar la información en este momento.");
      } catch (err) {
        console.error("Error fetching data:", err);
        setError(
          "Lo sentimos, ha ocurrido un error al conectar con el servidor de información.",
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-2xl p-10 mt-4 border border-gray-100">
      <div className="flex items-center gap-6 mb-8 pb-4 border-b">
        ...
        <h2 className="text-4xl font-black text-[#1c6c3e] uppercase">
          {section}
        </h2>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-4">
          ...
          <p className="text-[#1c6c3e] font-bold text-xl animate-pulse">
        ...
        <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed">
          ...
          <div className="mt-12 grid grid-cols-2 gap-4">
            <img
              src={`https://picsum.photos/seed/${section}-1/400/250`}
              alt="Zuheros 1"
              className="rounded-2xl shadow-md border-4 border-white"
            />
            <img
              src={`https://picsum.photos/seed/${section}-2/400/250`}
              alt="Zuheros 2"
              className="rounded-2xl shadow-md border-4 border-white"
            />
          </div>
